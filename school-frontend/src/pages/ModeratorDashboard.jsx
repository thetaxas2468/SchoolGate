  ...
  return (
    <div>
      <div className="stats-grid">
        <div className="stat-card accent1"><span className="stat-icon">🏫</span><div className="stat-value">{classes.length}</div><div className="stat-label">الفصول</div></div>
        <div className="stat-card accent3"><span className="stat-icon">👥</span><div className="stat-value">{students.length}</div><div className="stat-label">الطلاب</div></div>
        <div className="stat-card accent2"><span className="stat-icon">👩‍🏫</span><div className="stat-value">{teachers.length}</div><div className="stat-label">المعلمون</div></div>
        <div className="stat-card accent4"><span className="stat-icon">📋</span><div className="stat-value">{messages.length}</div><div className="stat-label">إجمالي الأذونات</div></div>
      </div>
      {todayMsgs.length > 0 && (
        <div className="card" style={{marginBottom: 24}}>
          <div className="card-header">
            <h3>📅 اذونات اليوم ({todayMsgs.length})</h3>
          </div>
      ...
      <div className="card">
        <div className="card-header"><h3>📋 أذونات حديثة</h3></div>
        <div style={{padding: '16px 24px'}}>
          {messages.slice(0, 8).map(m => <MsgCard key={m._id} message={m} showTeacher />)}
          {messages.length === 0 && <div className="empty-state"><span className="empty-state-icon">📭</span><p>لا توجد رسائل بعد.</p></div>}
        </div>
      </div>
    </div>
  );
...
function ClassesTab({ classes, students, teachers, reload }) {
  ...
  return (
    <div>
      <div className="card" style={{marginBottom: 24, maxWidth: 480}}>
        ...
        <div className="card-body">
          <form onSubmit={createClass} style={{display: 'flex', gap: 12}}>
            <input className="form-input" placeholder="اسم الفصل (مثال: ١٠-أ)" value={newClassName} onChange={e => setNewClassName(e.target.value)} required />
      ...
      {classes.length === 0 ? (
        <div className="empty-state"><span className="empty-state-icon">🏫</span><p>لا توجد فصول بعد. أنشئ فصلاً أعلاه.</p></div>
      ) : (
              ...
              <div className="class-card-body">
                ...
                <div className="form-group" style={{marginBottom: 12}}>
                  ...
                  <select className="form-select" value="" onChange={e => { if(e.target.value) addStudentToClass(cls._id, e.target.value); }}>
                    <option value="">— إضافة طالب إلى الصف —</option>
                    {unassignedStudents.map(s => <option key={s._id} value={s._id}>{s.name}</option>)}
                  </select>
                </div>
                <div>
                  <div className="form-label" style={{marginBottom: 8}}>الطلاب ({cls.students?.length || 0})</div>
                  {(cls.students?.length === 0) && <span style={{fontSize: 12, color: 'var(--text3)'}}>لا يوجد طلاب بعد</span>}
                  {cls.students?.map(s => (
                    <div key={s._id} className="student-chip">
                      👤 {s.name}
                      <button className="remove-btn" onClick={() => removeStudentFromClass(cls._id, s._id)} title="Remove">×</button>
                    </div>
                  ))}
                </div>
              </div>
      ...
      <div className="card" style={{marginBottom: 24, maxWidth: 480}}>
        ...
        <div className="card-body">
          <form onSubmit={create} style={{display: 'flex', gap: 12}}>
            <input className="form-input" placeholder="اسم الطالب بالكامل" value={name} onChange={e => setName(e.target.value)} required />
      ...
      <div className="card">
        <div className="card-header">
          ...
          <div className="search-wrap">
            ...
            <input className="search-input" placeholder="البحث..." value={search} onChange={e => setSearch(e.target.value)} />
          </div>
        </div>
        <div className="table-wrap">
          {filtered.length === 0 ? (
            <div className="empty-state"><span className="empty-state-icon">👤</span><p>لا يوجد طلاب.</p></div>
          ) : (
            <table>
              <thead>
                  ...
                  <tr key={s._id}>
                    ...
                    <td>
                      {deleteConfirm === s._id ? (
                        ...
                        <button className="btn btn-danger btn-sm" onClick={() => setDeleteConfirm(s._id)}>🗑️ Delete</button>
                      )}
                    </td>
                  </tr>
                ))}
    ...
    <div>
      <div style={{marginBottom: 20, display:'flex', gap:12}}>
        <button className="btn btn-primary" onClick={() => setShowForm(!showForm)}>
          {showForm ? '✕ إلغاء' : '➕ اضافة معلم جديد'}
        </button>
      </div>
      {showForm && (
        <div className="card" style={{marginBottom: 24, maxWidth: 480}}>
      ...
      <div className="card">
        <div className="card-header"><h3>جميع المعلمين ({teachers.length})</h3></div>
        <div className="table-wrap">
          {teachers.length === 0 ? (
            ...
            <table>
              <thead>
                <tr><th>رقم</th><th>الاسم</th><th>البريد الإلكتروني</th><th>الفصل</th><th>الإجراءات</th></tr>
              </thead>
              <tbody>
                {teachers.map((t, i) => (
                  <tr key={t._id}>
                    ...
                    <td>{t.assignedClassId ? <span className="badge badge-class">{t.assignedClassId.name}</span> : <span style={{color:'var(--text3)',fontSize:12}}>غير معين</span>}</td>
                    <td>
                      {deleteConfirm === t._id ? (
                        <div style={{display:'flex',gap:6,alignItems:'center'}}>
                          <span style={{fontSize:12,color:'var(--text3)'}}>Sure?</span>
                        ...
                        <button className="btn btn-danger btn-sm" onClick={() => setDeleteConfirm(t._id)}>🗑️ Delete</button>
                      )}
                    </td>
                  </tr>
                ))}
...
function MessagesTab({ messages, reload }) {
  ...
  return (
    <div className="card">
      <div className="card-header">
        <h3>جميع أذونات الخروج ({messages.length})</h3>
        <div className="search-wrap">
          <span className="search-icon">🔍</span>
      ...
      <div style={{padding: '16px 24px'}}>
        {filtered.length === 0 ? (
          <div className="empty-state"><span className="empty-state-icon">📭</span><p>No messages found.</p></div>
        ) : (
          filtered.map(m => (
            <div key={m._id}>
              {deleteConfirm === m._id && (
